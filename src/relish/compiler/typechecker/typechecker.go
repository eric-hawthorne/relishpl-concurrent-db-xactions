...
// DEPRECATED PACKAGE - SEE relish/compiler/types INSTEAD.
...
//
type Importer func(path string) ([]byte, error)
...
//
func CheckPackage(fset *token.FileSet, pkg *ast.Package, importer Importer) error {
	var tc typechecker
	tc.fset = fset
...
//
func CheckFile(fset *token.FileSet, file *ast.File, importer Importer) error {
