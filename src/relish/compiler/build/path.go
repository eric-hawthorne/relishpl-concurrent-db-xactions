...
var (
	ErrNotFound     = errors.New("relish/compiler/build: package could not be found locally")
	ErrTreeNotFound = errors.New("relish/compiler/build: no valid GOROOT or GOPATH could be found")
)
...
func init() {
	...
	t, err := newTree(root)
	if err != nil {
		log.Printf("relish/compiler/build: invalid GOROOT %q: %v", root, err)
	} else {
		t.Goroot = true
		Path = []*Tree{t}
	}
	...
	for _, p := range filepath.SplitList(os.Getenv("GOPATH")) {
		...
		t, err := newTree(p)
		if err != nil {
			log.Printf("relish/compiler/build: invalid GOPATH %q: %v", p, err)
			continue
		}
		Path = append(Path, t)
